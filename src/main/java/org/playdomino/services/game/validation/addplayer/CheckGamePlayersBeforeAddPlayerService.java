package org.playdomino.services.game.validation.addplayer;

import lombok.RequiredArgsConstructor;
import org.playdomino.components.messages.MessagesComponent;
import org.playdomino.exceptions.game.DominoGameException;
import org.playdomino.exceptions.game.DominoGameExceptionConstants;
import org.playdomino.models.auth.User;
import org.playdomino.models.game.DominoGame;
import org.playdomino.models.game.dto.AddPlayerToGame;
import org.playdomino.services.financial.WalletService;
import org.springframework.stereotype.Service;
import org.springframework.transaction.annotation.Transactional;

@Service
@RequiredArgsConstructor
public class CheckGamePlayersBeforeAddPlayerService implements BeforeAddPlayerService {

    private final MessagesComponent messagesComponent;

    @Transactional(readOnly = true)
    public void process(AddPlayerToGame playerToGame) {
        final DominoGame game = playerToGame.getGame();
        final User user = playerToGame.getUser();
        if (game.containsPlayer(user)) {
            throw new DominoGameException(DominoGameExceptionConstants.USER_ALREADY_JOINED_GAME, messagesComponent.getMessage(DominoGameExceptionConstants.USER_ALREADY_JOINED_GAME));
        }
    }
}
