...
import org.playdomino.exceptions.auth.InvalidAuthenticationException;
...
public class UserTokenServiceImpl implements UserTokenService{
    ...
    public JwtResponse getToken(UserToken login) {
        return providers.stream().filter(it -> it.accepts(login)).findFirst().orElseThrow(() -> new InvalidAuthenticationException(messagesComponent.getMessage(AuthExceptionConstants.USER_INVALID_AUTHENTICATION))).getToken(login);
    }
}
